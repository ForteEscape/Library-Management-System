...
public class BookService {
   ...
   */
  @Transactional
  public BookServiceResponseDto createNewBook(BookServiceRequestDto request) {
   ...
   * @return 결과 객체
   */
  public ArrayResponseWrapper<Page<BookServiceResponseDto>> searchBook(BookSearchCond cond,
      Pageable pageable) {
    Page<BookServiceResponseDto> books = bookRepository.bookSearch(cond, pageable);

    ArrayResponseWrapper<Page<BookServiceResponseDto>> result = new ArrayResponseWrapper<>();
   ...
   * @return 결과 객체
   */
  public ArrayResponseWrapper<Page<BookServiceResponseDto>> searchBookByTypeCode(int startCode,
      int endCode, Pageable pageable) {

    if (isValidRange(startCode, endCode)){
      throw new InvalidArgumentException(INVALID_RANGE);
    }

    Page<BookServiceResponseDto> books = bookRepository.findAllByBookTypeCode(startCode,
        endCode, pageable);
  ...
  @Transactional
  public BookServiceResponseDto updateBookData(Long bookId, BookServiceUpdateRequestDto request) {
    Book book = bookRepository.findById(bookId)
        .orElseThrow(() -> new NoSuchElementExistsException(BOOK_NOT_EXISTS));
